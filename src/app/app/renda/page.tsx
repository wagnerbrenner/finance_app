...
import { accounts, categories, transactions, uberPeriods } from "@/server/db/schema";
...
function uberPeriodEditFields(row: {
  source: string;
  periodMonth: string;
  daysWorked: number;
  grossRevenue: string;
  ...
  return (
    <>
      ...
      <FormField
        ...
        step="0.01"
        required
        defaultValue={String(toNumber(row.grossRevenue))}
      />
      <FormField
        name="daysWorked"
        ...
        min="0"
        step="1"
        defaultValue={String(row.daysWorked)}
      />
      <FormField
        name="hoursWorked"
        ...
        min="0"
        step="0.01"
        defaultValue={String(toNumber(row.hoursWorked))}
      />
      <FormField
        name="kmDriven"
        label="Quilômetros"
        ...
        min="0"
        step="0.01"
        defaultValue={String(toNumber(row.kmDriven))}
      />
      <FormField
        name="fuelCost"
        label="Combustível"
        ...
        min="0"
        step="0.01"
        defaultValue={String(toNumber(row.fuelCost))}
      />
      <FormField
        name="tolls"
        label="Pedágios"
        ...
        min="0"
        step="0.01"
        defaultValue={String(toNumber(row.tolls))}
      />
      <FormField
        name="wash"
        label="Lavagem"
        ...
        min="0"
        step="0.01"
        defaultValue={String(toNumber(row.wash))}
      />
      <FormField
        name="maintenance"
        label="Manutenção / impostos"
        type="number"
        min="0"
        step="0.01"
        defaultValue={String(toNumber(row.maintenance))}
      />
      <FormField
        name="otherCosts"
        label="Outros custos"
        ...
        min="0"
        step="0.01"
        defaultValue={String(toNumber(row.otherCosts))}
      />
      <FormField
        name="notes"
        label="Observações"
...
export default async function RendaPage() {
  ...
  const [salaryRows, periodRows, accountRows, categoryRows] = await Promise.all([
    db
      .select()
      .from(transactions)
      .where(
  ...
  const periodActions = (row: (typeof periodRows)[number]) => (
    <div className="inline-flex shrink-0 items-center gap-0.5">
      <EditEntityDialog
        title="Editar período"
        path="/app/renda"
        action={updateUberPeriod}
        id={row.id}
      >
        {uberPeriodEditFields(row)}
      </EditEntityDialog>
      <ConfirmDeleteButton
        id={row.id}
        path="/app/renda"
                  ...
                  {items.map((row) => {
                    ...
                    return (
                      <TableRow key={row.id}>
                        ...
                        <TableCell className="text-right">{periodActions(row)}</TableCell>
                      </TableRow>
                    );
                  })}
                </TableBody>
                          ...
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <div className="inline-flex shrink-0 items-center gap-0.5">
                              <EditEntityDialog
                                ...
                                id={row.id}
                              >
                                {salaryEditFields(row, accountOptions, categoryOptions)}
                              </EditEntityDialog>
                              <ConfirmDeleteButton
                                id={row.id}
                                path="/app/renda"
