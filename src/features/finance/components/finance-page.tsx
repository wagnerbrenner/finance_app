...
type Action = (data: FormData) => Promise<{ ok: boolean; error?: string }> | void | Promise<void>;
const asFormAction = (action: Action) => async (data: FormData) => {
  await action(data);
};
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <CardContent className="space-y-3">
          {rows.length === 0 ? <p className="text-sm text-muted-foreground">Nenhum registro ainda.</p> : rows.map((row) => (
            <div className="flex items-center justify-between rounded-lg border p-3" key={row.id}>
              ...
              <div className="flex items-center gap-3"><span className="font-medium">{row.amount}</span><form action={asFormAction(deleteAction)}><input type="hidden" name="id" value={row.id} /><input type="hidden" name="path" value={path} /><Button variant="ghost" type="submit">Excluir</Button></form></div>
            </div>
          ))}
          {children}
        </CardContent>
      ...
      <Card className="h-fit">
        ...
        <CardContent>
          <form action={asFormAction(createAction)} className="grid gap-3">
