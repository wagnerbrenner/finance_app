...
/** Confirmação de cadastro via API Resend (não depende do SMTP do Supabase). */
export async function sendSignupConfirmationEmail(to: string, confirmUrl: string) {
  ...
  const html = brandEmailShell(
    "Confirme seu e-mail",
    `<p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:#d4d4d8;">
      Bem-vindo ao Finance OS. Clique no botão abaixo para confirmar sua conta e começar a organizar suas finanças.
    </p>
    <p style="margin:28px 0;">
      <a href="${escapeHtml(confirmUrl)}" style="display:inline-block;background:#14b8a6;color:#042f2e;font-weight:700;text-decoration:none;padding:14px 24px;border-radius:10px;font-size:15px;">
        Confirmar e-mail
      </a>
    </p>
    ...
    <p style="margin:0;font-size:12px;line-height:1.5;word-break:break-all;">
      <a href="${escapeHtml(confirmUrl)}" style="color:#2dd4bf;">${escapeHtml(confirmUrl)}</a>
    </p>`,
  );
  ...
  const { data, error } = await resend.emails.send({
    from: fromAddress(),
    to,
    subject: "Confirme sua conta no Finance OS",
    html,
  });
